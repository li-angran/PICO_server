...
# %%
    ...
    "/mnt/nas00/lk/pico/Experiments/acc_test/angular-velocity-test/20_angular/20bright_calcium/frames",
"/mnt/nas00/lk/pico/Experiments/acc_test/angular-velocity-test/20_angular/20dark_calcium/frames"]
, help='Path to the data directory')
    
    parser.add_argument('--out_path', type=str, default='/mnt/nas00/lk/pico/Experiments/acc_test/angular-velocity-test/20_angular/Analysis', help='Path to the output directory')
    ...
    parser.add_argument('--minArea', type=int, default=50, help='Minimum neuron area')
    ...
    parser.add_argument('--thresh_COM', type=int, default=10, help='Merge threshold')
...
def PICO_processing(args):
    ...
    if not jump_to_rmbg:
        ...
        # %% Frame reading and counting for multiple directories
        frame_info = []  # Store info about each directory and its frame count
        video = []  # Store all frames from all directories
        for path in data_path:
            frame_count = len(glob.glob(os.path.join(path, '*.jpg')))
            frame_info.append((path, frame_count))
            logger.info(f'Reading frames from {path} - {frame_count} frames detected.')
            
            # Read all frames in the directory
            video_tmp = []
            for i in tqdm(range(frame_count)):
                frame_path = os.path.join(path, f'frame_{i}.jpg')
                img = cv2.imread(frame_path)
                img = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
                video_tmp.append(img)
            video.extend(video_tmp)

        # Save frame info to a file in the output directory
        frame_info_path = os.path.join(out_path, 'frame_info.txt')
        with open(frame_info_path, 'w') as f:
            for path, count in frame_info:
                f.write(f'{path}: {count} frames\n')
        
        logger.info(f'Saved frame info to {frame_info_path}')
        ...
        Correction_Map = np.load('/home/angran/BBNC/code/PICO_ca_processing/utils/CorrectionMap.npz')
        ...
        model = YOLO("/home/angran/BBNC/code/PICO_ca_processing/utils/yolo_v8s.pt")
        ...
        vessel_img, vessel_mask = get_vessel_mask(norm_img, '/home/angran/BBNC/code/PICO_ca_processing/utils/vessel_model.pt')
