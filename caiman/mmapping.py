...
import caiman as cm
from caiman.paths import fn_relocated, memmap_frames_filename
...
def save_memmap(filenames: List[str],
    ...
    if len(filenames) > 1:
        ...
        for idx, f in enumerate(filenames):
            ...
            if is_3D:
                ...
                if isinstance(f, (basestring, list)):
                    Yr = cm.load(fn_relocated(f), fr=1, in_memory=True, var_name_hdf5=var_name_hdf5)
                else:
                    Yr = cm.movie(f)
            ...
            if idx == 0:
                ...
                if isinstance(f, str):
                    fname_tot = fn_relocated(os.path.join(os.path.split(f)[0], fname_tot))
                if len(filenames) > 1:
                    big_mov = np.memmap(fn_relocated(fname_tot),
                                        mode='w+',
                                        dtype=np.float32,
        ...
        fname_new = fn_relocated(fname_tot + f'_frames_{Ttot}_.mmap')
        try:
...
def save_tif_to_mmap_online(movie_iterable, save_base_name='YrOL_', order='C', add_to_movie=0, border_to_0=0) -> str:
    ...
    fname_tot = fn_relocated(memmap_frames_filename(save_base_name, dims[1:], dims[0], order))
