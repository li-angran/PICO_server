...
# %%
    ...
    parser.add_argument('--data_path', type=str, default="/mnt/nas00/lk/pico/Experiments/maze/20241106maze/8#/calcium01/frames", help='Path to the data directory')
    parser.add_argument('--out_path', type=str, default="/mnt/nas00/lk/pico/Experiments/maze/20241106maze/8#/calcium01/Analysis", help='Path to the output directory')
...
def PICO_processing(args):
    ...
    if not jump_to_rmbg:
        ...
        mc_all_path = os.path.join(mc_out, "all")

        frame_number = len(os.listdir(mc_all_path))
        ...
        for i in tqdm(range(frame_number)):
            ...
            img = cv2.imread(os.path.join(mc_all_path, f"frame_{i}.tif"), cv2.IMREAD_GRAYSCALE).astype(np.float16)
