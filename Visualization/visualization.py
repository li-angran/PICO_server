...
def plot_cm(cm, d1, d2, save_path, save_svg_path):
    plt.figure(figsize=(5, 5))
    plt.scatter(cm[:, 0], cm[:, 1], c='blue', marker='o', label='Neurons')
    plt.title('Center of Mass of Neurons')
...
def view_patches(Yr, A, C, b, f, d1, d2, YrA=None, secs=1):
    ...
    sys.stdout.flush()
    for i in range(nr + 1): # for each of neurons
        if i < nr:
